...
export const GET_ASSIGNMENTS_ID_QUERY_KEY: QueryKey = `assignmentsClassesSummary`;
...
export function useGetAssignmentClassesSummary (request: GetAssignmentsRequest, options?: RequestConfigQueryOptions<GetAssignmentsResponse>) {
    ...
    return useQuery([ GET_ASSIGNMENTS_ID_QUERY_KEY, request ], () => getAssignmentClassesSummary(axiosClient, request, options?.config), options?.queryOptions);
...
export interface ReportLiveClass {
    absent: boolean;
    assessment_id: string;
...
export const GET_LEARNING_OUTCOMES_ID_QUERY_KEY: QueryKey = `learningOutComes`;
...
export function useGetLearningOutcomes (request: GetLearningOutComesRequest, options?: RequestConfigQueryOptions<GetLearningOutComesResponse[]>) {
    const { axiosClient } = useCmsApiClient();
    return useQuery([ GET_LEARNING_OUTCOMES_ID_QUERY_KEY, request ], () => getLearningOutcomes(axiosClient, request, options?.config), options?.queryOptions);
...
export interface ReportAssignmentLabel {
...
export interface ReportAttendanceLabel {
...
export interface ReportLearningOutComeAchivement {
...
export interface GetAppInsightMessagesResponse {
    ...
    assignment_label_params: ReportAssignmentLabel;
    attedance_label_id: string;
    attedance_label_params: ReportAttendanceLabel;
    learning_outcome_achivement_label_id: string;
    learning_outcome_achivement_label_params: ReportLearningOutComeAchivement;
}
...
export const GET_INSIGTH_MESSAGE_ID_QUERY_KEY: QueryKey = `getAppInsightMessage`;
...
export function useGetAppInsightMessage (request: GetAppInsightMessagesRequest, options?: RequestConfigQueryOptions<GetAppInsightMessagesResponse>) {
    const { axiosClient } = useCmsApiClient();
    return useQuery([ GET_LEARNING_OUTCOMES_ID_QUERY_KEY, request ], () => getAppInsightMessage(axiosClient, request, options?.config), options?.queryOptions);
